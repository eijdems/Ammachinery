...
class MagicZoomPlusModuleCoreClass
{
    ...
    public function getHeadersTemplate($jsPath = '', $cssPath = null)
    {
        ...
        $headers = [];
        $headers[] = '<!-- Magic Zoom Plus Magento 2 module version v1.5.47 [v1.6.79:v5.2.9] -->';
    ...
    public function getMainTemplate($params)
    {
        ...
        return "<a id=\"MagicZoomPlusImage{$id}\" {$dataImage2x} class=\"MagicZoom itemm\" href=\"{$img}\"{$group}{$link}{$title}{$options}><img itemprop=\"image\" src=\"{$thumb}\" {$thumb2x}  alt=\"{$alt}\"{$width}{$height} /></a>{$message}";
    }
