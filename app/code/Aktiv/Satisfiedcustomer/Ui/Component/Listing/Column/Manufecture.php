...
class Manufecture extends \Magento\Ui\Component\Listing\Columns\Column
{
    ...
    public function prepareDataSource(array $dataSource)
    {
        if (isset($dataSource['data']['items'])) {
            foreach ($dataSource['data']['items'] as & $item) {
                $optionId =  $item['manufectureid'];
                $attribute = $this->productModel->getResource()->getAttribute('manufacturer');
                $optionText='';
                if ($attribute->usesSource()) {
                    $optionText = $attribute->getSource()->getOptionText($optionId);
                }
                $item['manufectureid'] = $optionText;
