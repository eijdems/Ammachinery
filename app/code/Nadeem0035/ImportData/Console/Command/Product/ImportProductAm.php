...
use Nadeem0035\ImportData\Console\Command\AbstractImportCommandAm;

class ImportProductAm extends AbstractImportCommandAm
{
    /**
    ...
    {
        $this->setName('importdata:products:importproductam')
            ...
            if (!empty($products)) {
                foreach ($products as $product) {
                    ...
                    if ($product->ServiceNumber > 0) {
                        ...
                        if($product->SubGroupCode == "01" ||
                             $product->SubGroupCode == "02" ||
                             $product->SubGroupCode == "03"
                         ){
                            $subCategoryName = $this->getCorrectCategory((@$product->SubGroup ?: ''));
                        }
                        else if ($product->SubGroupCode=="10") {
                            $subCategoryName = 'Innoculant applicator';
                        } else if ($product->SubGroupCode=="B3") {
                            //print_r($product);
                            $subCategoryName = 'Direct discs';
                        }
                        ...
                        $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/customAm.log');
                        $logger = new \Zend_Log();
                        $logger->addWriter($writer);
                        $logger->info('======================MAgento Ammachinery Data');
                        $logger->info(print_r($default_data, true));



                        if ($product->WebsiteConfigurationText != NULL) {
                            $default_data['short_description'] = $product->WebsiteConfigurationText;
                        }
                        $tierprice = [];
                        if ($tradePrice) {
                            $tierprice[0] = array('website_id' => 2,'cust_group' => 4, 'price_qty' => 1 , 'price' => $tradePrice );
                        }
                        
                        if ($subCategoryName == 'Innoculant applicator') {
                            $default_data['categories'] = "AM Category/Upgrades";
                        } else if (!empty($categoryName)) {
                            $default_data['categories'] = "AM Category/Stock/" . $categoryName;
                        } else {
                            $default_data['categories'] = "AM Category";
                        }


                        if (!empty($subCategoryName)) {
                            $default_data['subcategories'] = $subCategoryName;
                        } else {
                            $default_data['subcategories'] = "";
                        }
                        
                        $defaultAttributes['type'] = preg_replace('/[^A-Za-z0-9. -]/', '', $product->Type);
                        ...
                        $default_data['tierPrice']  = $tierprice;
                        $data[] = $default_data;
                    }
                }
